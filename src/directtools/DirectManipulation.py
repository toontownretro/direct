...
class ObjectHandles(NodePath, DirectObject):
    def __init__(self, name='objectHandles'):
        ...
        model = loader.loadModel('models/misc/objectHandles')
        self.assign(model)
        self.setName(name)
        ...
        if self.xHandles.isEmpty():
            print("**ERROR: Failed to get X handle from object handles model!")
            self.xHandles = self.find('**/Y')
            self.xPostGroup = self.xHandles.find('**/y-post-group')
            self.xPostCollision = self.xHandles.find('**/y-post')
            self.xRingGroup = self.xHandles.find('**/y-ring-group')
            self.xRingCollision = self.xHandles.find('**/y-ring')
            self.xDiscGroup = self.xHandles.find('**/y-disc-group')
            self.xDisc = self.xHandles.find('**/y-disc-visible')
            self.xDiscCollision = self.xHandles.find('**/y-disc')
            self.xScaleGroup = deepcopy(self.xPostGroup)
            self.xScaleGroup.setName('x-scale-group')
            self.xScaleCollision = self.xScaleGroup.find('**/y-post')
